...
function getDayName(date) {
  ...
  const indexDayInWeek = new Date(date).getDay();
  return weekDays[indexDayInWeek];
}
...
function getNextFriday(date) {
  const indexDayInWeek = date.getDay();
  const diffDays = 5 - indexDayInWeek + (indexDayInWeek >= 5 ? 7 : 0);
...
function isDateInPeriod(date, period) {
  ...
  const startP = new Date(period.start).getTime();
  const endP = new Date(period.end).getTime();
  if (currentDate >= startP && currentDate <= endP) {
    return true;
  }
  return false;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    const currentDate = new Date(year, month - 1, day).getDay();
...
function getWeekNumberByDate(date) {
  ...
  const weekNumber = Math.ceil((diff + daysOffset + 1) / 7);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const startP = new Date(period.start.split('-').reverse().join('-'));
  const endP = new Date(period.end.split('-').reverse().join('-'));
  const resultArray = [];
  let day = startP;
  let isWorkDay = true;
  ...
  while (day <= endP) {
    if (isWorkDay) {
      for (let i = 0; i < countWorkDays && day <= endP; i += 1) {
        resultArray.push(day.toJSON());
